...
def qualification(entry,criteria):
    ...
    for key in scoring_weights.keys():
        ...
        weighted_criteria.append((weight, [token.strip() for token in key.split(',')]))
...
##            print('Entries present:', len(d['entries']))
    ...
    for named_category in txt_categories:
        ...
        output += '<span id="' + ''.join(named_category.split()) + '">' + '<br><br>' + '*'*10 + ' ' + named_category.upper() + ' ' + '*'*10 + '<br><br><br></span>'
        list_matches = list()
...
__categories__<br>
<br>
Date/Time of Last Update: __datetime__<br>
<br>
__output__<br>
<br>
</body>
...
fileOutput = open('index.html', 'w', encoding='utf-16')
